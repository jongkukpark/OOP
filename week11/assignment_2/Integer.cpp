...
int Integer::parse_string(const std::string& str) { //완
    /**
     * 문자열을 int로 변환하는 함수
     */
...
Integer::Integer(const std::string& str) { //완
    /**
...
Number *Integer::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		Integer* newNum = dynamic_cast<Integer*>(num);
		newNum->set_val(this->val() + newNum->val());
		delete(this);
		return newNum;
    } else if (num->types() == FLOAT) {
		Float* newNum = dynamic_cast<Float*>(num);
		newNum->set_val(this->val() + newNum->val());
		delete(this);
		return newNum;
    } else if (num->types() == COMPLEX) {
		Complex* newNum = dynamic_cast<Complex*>(num);
		newNum->set_val(this->val() + newNum->real(), newNum->imag());
		delete(this);
		return newNum;
    } else {
...
void Integer::set_val(int val) { //완
    /**
...
Number::type Integer::types() const { //완
    /**
...
std::string Integer::to_string() const { //완
    /**
     * int 값의 string
     */
